...
train_set = LSDLoader("/home/student/Documents/Learning-to-see-in-the-dark-CNN/dataset/train/Sony")

validation_set = LSDLoader("/home/student/Documents/Learning-to-see-in-the-dark-CNN/dataset/val/Sony")
...
num_epochs = 100
plotter = VisdomLinePlotter("Train_and_Validation")
    ...
    for batch_index, (inputs, targets) in enumerate(train_loader):
        ...
        if epoch > 0 and epoch % 10 == 0:
            ...
            for batch_index, (inputs, targets) in enumerate(validation_loader):
                ...
                opt.step()
                train_loss += loss.item()
            print("Validation Loss: %.10f " % (train_loss))
            plotter.plot('loss', 'eval', 'Train VS Eval loss', epoch, train_loss )
            ...
            ToImage.toimage(temp * 255, high=255, low=0, cmin=0, cmax=255).save(
                '%04d/00_train%d.jpg' % (epoch, batch_index))
        torch.save(network, 'LSD')
    print('Results after epoch %d' % (epoch + 1))

    print("Training Loss: %.10f " % (train_loss / (batch_index + 1)))
    plotter.plot('loss', 'train', 'Train VS Eval loss', epoch, train_loss / (batch_index + 1))
